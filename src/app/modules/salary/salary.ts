...
export class Salary implements OnInit {
    ...
    this.salaryService.generateSalary(request).subscribe({
      ...
      error: (err) => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Unable to Generate Salary',
          text: err.error?.message || err.error?.detail || err.error || 'Something went wrong.',
        });
  ...
  viewPayslip(salaryId: number): void {
    ...
    this.salaryService.getPayslip(salaryId).subscribe({
      ...
      error: (err) => {
        ...
        Swal.fire(
          'Error',
          err.error?.message || err.error?.detail || err.error || 'Unable to load payslip.',
          'error',
        );
      },
    });
  ...
  explainWithAi(salaryId: number): void {
    this.aiSalaryId = salaryId;
    ...
    this.salaryService.explainSalaryWithAi(salaryId).subscribe({
      next: (res: any) => {
      ...
      error: (err) => {
        console.error('AI payroll explanation error', err);
        ...
        Swal.fire(
          ...
          err.error?.message || err.error?.detail || 'Unable to generate salary explanation.',
          'error',
        );
